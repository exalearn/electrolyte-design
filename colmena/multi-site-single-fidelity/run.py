...
class Thinker(BaseThinker):
    ...
    def __init__(self, queues: ClientQueues,
        ...
        self.mpnns_paths = mpnns.copy()
        self.mpnns = [tf.keras.models.load_model(path, custom_objects=custom_objects) for path in tqdm(args.mpnn_model_files, desc='Loading models')]
        ...
        self.logger.info(f'Populated an initial database of {database.collection.count_documents({})} entries')

        # Get the target database size
        self.n_to_evaluate = n_to_evaluate

        # List the molecules that have already been searched
        self.already_searched = set(self.database.get_molecules({f'oxidation_potential.{self.property_name}': {'$exists': True}}))

        # Prepare search space
        self.mols = pd.read_csv(search_space)
        self.logger.info(f'Read in a dataset of {len(self.mols)} molecules')
        
        self.mols.drop_duplicates('inchi', inplace=True)
        self.logger.info(f'Trimmed down to {len(self.mols)} non-duplicate molecules')
    ...
    def submit_qc(self):
        ...
        with self.task_queue_lock:
            ...
            self.logger.info(f'Submitted {smiles} to simulate. Run score: {info["ucb"]:.4f}')
    ...
    def train_models(self):
        ...
        # Create proxies in a large batch
        ...
        input_key = 'identifier.smiles'
        output_key = f'oxidation_potential.{self.property_name}'
        train_data = self.database.get_training_set([input_key], [output_key])
        train_data = dict(zip(train_data[input_key], train_data[output_key]))
        if self.ps_names['train'] is not None:
    ...
    def update_weights(self, result: Result):
        ...
        if not result.success:
            ...
            self.logger.info(f'Saving updated model to {self.mpnns_paths[model_id]}')
            self.mpnns[model_id].save(self.mpnns_paths[model_id])
...
if __name__ == '__main__':
    ...
    group.add_argument('--ps-threshold', default=5000, type=int, help='Min size in bytes for transferring objects via ProxyStore')
    group.add_argument('--ps-file-dir', default='proxy-store-scratch', help='Filesystem directory to use with the ProxyStore file backend')
    ...
    if 'globus' in ps_backends:
        ...
        ps.store.init_store(ps.store.STORES.GLOBUS, name='globus', endpoints=endpoints, timeout=3600)
