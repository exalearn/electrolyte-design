...
def build_fn(atom_features: int = 128, message_steps: int = 8,
    ...
    if atomwise:
        output = Atomwise(n_in=atom_features, n_layers=output_layers, aggregation_mode=reduce_fn,
                          mean=mean, stddev=std, property='ip')
    else:
...
if __name__ == "__main__":
    ...
    train_loader = AtomsLoader(train_data, args.batch_size, shuffle=True, pin_memory=True, num_workers=2)
    ...
    valid_loader = AtomsLoader(valid_data, args.batch_size, pin_memory=True, num_workers=2)

    # Make the model
    mean, std = train_loader.get_statistics('ip', divide_by_atoms=args.atomwise)
    ...
    for batch in test_loader:
        ...
        # apply model
        pred = model(batch)
        y_pred.append(pred['ip'].detach().cpu().numpy())
        y_true.append(batch['ip'].detach().cpu().numpy())
